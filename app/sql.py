...
# Function to generate SQL query from the question using LLM
...
# Compress the data and generate the answer using LLM
# Represent in a better and human nderstandiabe format.
def data_comprehension(question, context):
    chat_completion = client_sql.chat.completions.create(
        ...
        model=os.environ['GROQ_MODEL'],
        temperature=0.2,
        max_tokens=1024  # Added a max token limit for cost and performance.
    )
    # Return the answer generated by the LLM
    return chat_completion.choices[0].message.content


# We generate SQL queri from the question using LLM, then we run the query on the database to get the data.
def sql_chain(question):
    # Generate SQL query from the question
    sql_query = generate_sql_query(question)
    # Extract SQL query from the response
    pattern = "<SQL>(.*?)</SQL>"
    # Find all matches of the pattern in the sql_query string
    ...
    # Run the SQL query on the database to get the data
